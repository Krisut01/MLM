<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ route('admin.landing.update') }}" enctype="multipart/form-data" class="bg-white rounded-2xl shadow p-6 space-y-6">
                @csrf

                <div>
                    <h3 class="text-lg font-semibold text-gray-900 mb-2">Hero</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Hero Title</label>
                            <input name="hero_title" value="{{ old('hero_title', $settings->hero_title) }}" class="w-full rounded-lg border-gray-300" />
                    ...
                    <div class="mt-4">
                        ...
                        <textarea name="hero_subtitle" rows="3" class="w-full rounded-lg border-gray-300">{{ old('hero_subtitle', $settings->hero_subtitle) }}</textarea>
                        @error('hero_subtitle') <div class="text-sm text-red-600 mt-1">{{ $message }}</div> @enderror
                    </div>
                </div>

                <div class="border-t border-gray-200 pt-6">
                    <h3 class="text-lg font-semibold text-gray-900 mb-2">RWA Section</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">RWA Title</label>
                            <input name="rwa_title" value="{{ old('rwa_title', $settings->rwa_title) }}" class="w-full rounded-lg border-gray-300" />
                            @error('rwa_title') <div class="text-sm text-red-600 mt-1">{{ $message }}</div> @enderror
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">RWA Image</label>
                    ...
                    <div class="mt-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1">RWA Body</label>
                        <textarea name="rwa_body" rows="6" class="w-full rounded-lg border-gray-300">{{ old('rwa_body', $settings->rwa_body) }}</textarea>
                        @error('rwa_body') <div class="text-sm text-red-600 mt-1">{{ $message }}</div> @enderror
                    </div>
                </div>

                <div class="border-t border-gray-200 pt-6">
                    <h3 class="text-lg font-semibold text-gray-900 mb-2">CTA</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">CTA Text</label>
                            <input name="cta_text" value="{{ old('cta_text', $settings->cta_text) }}" class="w-full rounded-lg border-gray-300" />
                        ...
                        <div>
                            ...
                            <input name="cta_link" value="{{ old('cta_link', $settings->cta_link) }}" class="w-full rounded-lg border-gray-300" />
                ...
                <div class="flex items-center justify-between pt-4">
                    <a href="{{ route('landing') }}" class="text-sm text-gray-600 underline">Preview landing page</a>
